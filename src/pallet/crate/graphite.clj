...
(defmethod-plan install ::virtualenv
  ...
  [{:keys [user owner group src-dir home url] :as settings}
...
(def-plan-fn install-graphite
  "Install graphite."
  [& {:keys [instance-id]}]
  [settings (get-settings :graphite {:instance-id instance-id})]
  (install :graphite instance-id))


;; (def-plan-fn install-gunicorn
;;   "Install gunicorn"
...
;;; # User
(def-plan-fn graphite-user
  "Create the graphite user"
  [{:keys [instance-id] :as options}]
  [{:keys [user owner group home]} (get-settings :graphite options)]
...
(def-plan-fn graphite-conf
  "Write all config files"
  [{:keys [instance-id] :as options}]
  [{:keys [carbon-config storage-aggregation storage-schemas home user
           owner group]
    :as settings}
  ...
  ;; (with-action-options {:sudo-user user :script-dir home}
  ;;   (exec-checked-script
  ;;    "Initialise database"
  ;;    ("source" (str ~home "/bin/activate"))
  ;;    ("python" "django-admin.py" "createsuperuser"
  ;;     "--username" "graphite"
  ;;     "--email" "graphite@graphite")))
  )

(def-plan-fn graphite-service
...
(def-plan-fn graphite-web-server
  "Start a graohite web service"
  [{:keys [instance-id] :as options}]
  [{:keys [home service user group webapp-bind-address]}
   (get-settings :graphite {:instance-id instance-id})]
...
(defn graphite
  ...
  (server-spec
   :phases
   {:settings (graphite-settings (merge settings options))
    :install (plan-fn
              (graphite-user options)
              (install-graphite :instance-id instance-id))
    :configure (plan-fn (graphite-conf options)
